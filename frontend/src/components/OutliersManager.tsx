...
export default function OutliersManager({ sessionId, onComplete }: OutliersManagerProps) {
  ...
  const actionColors: Record<string, string> = {
    cap: 'bg-blue-100 text-blue-700 border-blue-300',
    remove_rows: 'bg-red-100 text-red-700 border-red-300',
      ...
      <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-2xl p-8 border border-slate-200 animate-scale-in">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">Handle extreme values using IQR method</p>
            </div>
          </div>
          <button
        ...
        <div className="bg-emerald-50 rounded-xl p-4 mb-6 border border-emerald-200">
          <div className="flex items-start space-x-2">
            ...
            <div>
              <p className="text-sm text-emerald-800 font-medium mb-2">AI Suggestions Applied</p>
              <p className="text-xs text-emerald-600">
                Cap: Clip values to IQR bounds • Remove Rows: Delete outlier records ({'>'}10% outliers) • None: Keep as is
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3 mb-6">
          {Object.entries(suggestions).map(([id, sug]) => {
            ...
            return (
              <div
                key={id}
                className="bg-slate-50 rounded-xl p-4 border-2 border-transparent hover:border-emerald-200 transition-all duration-300 animate-slide-in"
              >
                  ...
                  <div className="flex-1">
                    <div className="flex items-center space-x-3 mb-2">
                      ...
                      <span className={`px-3 py-1 rounded-lg text-sm font-semibold ${
                        sug.outliers_pct > 10 ? 'bg-red-100 text-red-700' : 'bg-orange-100 text-orange-700'
                      }`}>
                        {sug.outliers_pct.toFixed(1)}% outliers
                      </span>
                    </div>

                    {customizing ? (
                      <div className="flex space-x-2">
                        {['cap', 'remove_rows', 'none'].map(action => (
                          <button
                            key={action}
                          ...
                          >
                            {action === 'remove_rows' ? 'Remove Rows' : action === 'none' ? 'Keep' : 'Cap Values'}
                          </button>
                        ))}
                      </div>
                    ) : (
                      <div className="flex items-center space-x-2">
                        ...
                        <span className={`px-3 py-1 rounded-lg text-sm font-medium ${actionColors[currentAction]}`}>
                          {currentAction === 'remove_rows' ? 'Remove Rows' : currentAction === 'none' ? 'Keep as is' : 'Cap to IQR bounds'}
                        </span>
                      </div>
                    )}
                  </div>
        ...
        <button
          onClick={handleApply}
          disabled={processing}
          className="w-full bg-gradient-to-r from-emerald-500 to-teal-500 text-white py-4 px-6 rounded-xl font-semibold shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-300 disabled:opacity-50 disabled:hover:scale-100 flex items-center justify-center space-x-2"
        >
          {processing ? (
            <>
