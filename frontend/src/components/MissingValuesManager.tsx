...
export default function MissingValuesManager({ sessionId, onComplete }: MissingValuesManagerProps) {
  ...
  const fetchSuggestions = async () => {
    try {
      const response = await fetch(`https://automl-1smu.onrender.com/suggestions/missing/${sessionId}`);
  ...
  const handleApply = async () => {
    setProcessing(true);
    try {
      await fetch('https://automl-1smu.onrender.com/clean/missing', {
        method: 'POST',
  ...
  const actionColors: Record<string, string> = {
    mean: 'bg-blue-100 text-blue-700 border-blue-300',
    median: 'bg-cyan-100 text-cyan-700 border-cyan-300',
    mode: 'bg-green-100 text-green-700 border-green-300',
    drop_col: 'bg-red-100 text-red-700 border-red-300',
  };
      ...
      <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-2xl p-8 border border-slate-200 animate-scale-in">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-slate-800">Missing Values</h2>
              <p className="text-sm text-slate-500">Handle missing data intelligently</p>
            </div>
          </div>
          <button
        ...
        <div className="bg-blue-50 rounded-xl p-4 mb-6 border border-blue-200">
          <div className="flex items-start space-x-2">
            ...
            <div>
              <p className="text-sm text-blue-800 font-medium mb-2">AI Suggestions Applied</p>
              <p className="text-xs text-blue-600">
                Mean: Symmetric distributions • Median: Skewed distributions • Mode: Categorical data • Drop: {'>'}50% missing
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3 mb-6">
          {Object.entries(suggestions).map(([id, sug]) => {
            ...
            return (
              <div
                key={id}
                className="bg-slate-50 rounded-xl p-4 border-2 border-transparent hover:border-blue-200 transition-all duration-300 animate-slide-in"
              >
                  ...
                  <div className="flex-1">
                    <div className="flex items-center space-x-3 mb-2">
                      ...
                      <span className="px-3 py-1 bg-orange-100 text-orange-700 rounded-lg text-sm font-semibold">
                        {sug.missing_pct.toFixed(1)}% missing
                      </span>
                    </div>

                    {customizing ? (
                      <div className="flex space-x-2">
                        {['mean', 'median', 'mode', 'drop_col'].map(action => (
                          <button
                            key={action}
                          ...
                          >
                            {action === 'drop_col' ? 'Drop Column' : action.charAt(0).toUpperCase() + action.slice(1)}
                          </button>
                        ))}
                      </div>
                    ) : (
                      <div className="flex items-center space-x-2">
                        ...
                        <span className={`px-3 py-1 rounded-lg text-sm font-medium ${actionColors[currentAction]}`}>
                          {currentAction === 'drop_col' ? 'Drop Column' : `Fill with ${currentAction}`}
                        </span>
                      </div>
                    )}
                  </div>
        ...
        <button
          onClick={handleApply}
          disabled={processing}
          className="w-full bg-gradient-to-r from-blue-500 to-cyan-500 text-white py-4 px-6 rounded-xl font-semibold shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-300 disabled:opacity-50 disabled:hover:scale-100 flex items-center justify-center space-x-2"
        >
          {processing ? (
            <>
