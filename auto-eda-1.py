...
def drop_columns_by_id(df, df_info):
...
def handle_missing_values(df, df_info):
    ...
    for action, col_ids in user_plan.items():
        valid_ids = [cid for cid in col_ids if cid in all_missing_ids]
        for col_id in valid_ids:
            col_name = updated_info[updated_info['id'] == col_id]['column_name'].iloc[0]
            dtype = updated_info[updated_info['id'] == col_id]['dtype'].iloc[0]
            ...
            if action in ['mean', 'median'] and is_categorical:
                ...
                if convert == 'y':
                    try:
                        updated_df[col_name] = pd.to_numeric(updated_df[col_name], errors='raise')
                        print(f"Converted '{col_name}' to numeric.")
                        updated_info.loc[updated_info['column_name'] == col_name, 'dtype'] = updated_df[col_name].dtype
                    except:
            ...
            if action == 'mean':
                val = updated_df[col_name].mean()
                if not np.isnan(val):
                    updated_df[col_name] = updated_df[col_name].fillna(val)
                    print(f"Filled with mean: {val:.2f}")
            
            elif action == 'median':
                val = updated_df[col_name].median()
                if not np.isnan(val):
                    updated_df[col_name] = updated_df[col_name].fillna(val)
                    print(f"Filled with median: {val:.2f}")
            
            elif action == 'mode':
                mode_val = updated_df[col_name].mode()
                if not mode_val.empty:
                    updated_df[col_name] = updated_df[col_name].fillna(mode_val.iloc[0])
                    print(f"Filled with mode: {mode_val.iloc[0]}")
            
            elif action == 'drop_col':
                updated_df.drop(columns=[col_name], inplace=True)
                updated_info = updated_info[updated_info['id'] != col_id].reset_index(drop=True)
    ...
    if uncovered_ids:
        ...
        while True:
            ...
            if choice == '1':
                ...
                # Apply extra plan (same code as above, but only on uncovered)
                for action, col_ids in extra_plan.items():
                    valid_extra = [cid for cid in col_ids if cid in uncovered_ids]
                    for col_id in valid_extra:
                        # Repeat the apply logic here (same as above block)
                        col_name = updated_info[updated_info['id'] == col_id]['column_name'].iloc[0]
                        dtype = updated_info[updated_info['id'] == col_id]['dtype'].iloc[0]
                        is_categorical = is_object_dtype(dtype) or isinstance(dtype, pd.CategoricalDtype)
                        
                        print(f"\nApplying '{action}' to uncovered ID {col_id} ({col_name})")
                        # ... (same validation and apply code as above)
                        # For brevity, copy-paste the apply block here in your actual script
                # After extra, re-check if any still uncovered
                remaining_uncovered = uncovered_ids - covered_ids  # Update covered_ids with extra
                if remaining_uncovered:
                    print("Some columns still not covered.")
                    continue
                else:
                    break
                ...
                for col_id in uncovered_ids:
                    action = global_suggestion.get(col_id, 'median')  # fallback
                    # Apply the same logic as main plan
                    col_name = updated_info[updated_info['id'] == col_id]['column_name'].iloc[0]
                    print(f"\nApplying suggested '{action}' to ID {col_id} ({col_name})")
                    # ... (same apply code)
                break
...
# Add this new function to your script (below handle_missing_values)

def detect_and_handle_outliers(df, df_info):
    """
    Interactive outlier detection & handling — same flow as missing values.
    - Detects outliers using IQR method
    - Gives global suggestion
    - User approves or provides custom plan
    - Handles missed columns with 3 options
    """
    ...
    if approve == 'y':
        ...
        user_input = input("Your plan: ").strip()
        user_plan = parse_outlier_plan(user_input)  # We'll define this below
    ...
    # Apply user's plan (or approved suggestion)
    for action, col_ids in user_plan.items():
        valid_ids = [cid for cid in col_ids if cid in all_outlier_ids]
        for col_id in valid_ids:
            col_name = updated_info[updated_info['id'] == col_id]['column_name'].iloc[0]
            ...
            if action == 'cap':
                updated_df[col_name] = updated_df[col_name].clip(lower=lower, upper=upper)
                print(f"  Capped to [{lower}, {upper}]")
            
            elif action == 'remove_rows':
                initial_rows = len(updated_df)
                mask = (updated_df[col_name] >= lower) & (updated_df[col_name] <= upper)
                updated_df = updated_df[mask].reset_index(drop=True)
                removed = initial_rows - len(updated_df)
    ...
    if uncovered_ids:
        ...
        while True:
            ...
            if choice == '1':
                ...
                # Apply extra_plan (same logic as main plan)
                for action, col_ids in extra_plan.items():
                    valid_extra = [cid for cid in col_ids if cid in uncovered_ids]
                    for col_id in valid_extra:
                        ...
                        bounds = outlier_cols_info[outlier_cols_info['id'] == col_id][['lower_bound', 'upper_bound']].iloc[0]
                        lower, upper = bounds['lower_bound'], bounds['upper_bound']
                        print(f"\nApplying extra '{action}' to ID {col_id} ({col_name})")
                        if action == 'cap':
                            updated_df[col_name] = updated_df[col_name].clip(lower=lower, upper=upper)
                        elif action == 'remove_rows':
                            updated_df = updated_df[(updated_df[col_name] >= lower) & (updated_df[col_name] <= upper)].reset_index(drop=True)
                        elif action == 'none':
                            pass
                break
                ...
                print("Applying original global suggestions to uncovered columns...")
                for col_id in uncovered_ids:
                    action = global_suggestion.get(col_id, 'cap')
                    col_name = outlier_cols_info[outlier_cols_info['id'] == col_id]['column_name'].iloc[0]
                    bounds = outlier_cols_info[outlier_cols_info['id'] == col_id][['lower_bound', 'upper_bound']].iloc[0]
                    lower, upper = bounds['lower_bound'], bounds['upper_bound']
                    print(f"\nApplying suggested '{action}' to ID {col_id} ({col_name})")
...
def univariate_analysis(df, df_info):
    numeric_cols = df.select_dtypes(include=['float64', 'int64']).columns
    if len(numeric_cols) == 0:
...
def bivariate_analysis(df, df_info):
    """
    Bivariate Analysis with smart flow:
    - Correlation heatmap (Pearson/Spearman)
    - Pairplot (with optional hue if categorical)
    - User selects options once
    - Choose save folder
    - Viewing/saving mode like univariate:
        1. View → save ALL at end
        2. Direct save (no view)
        3. View each → decide save individually
    """
    print("\n" + "="*60)
...
def handle_high_correlation(df, df_info, threshold=0.90):
    """
    Detects highly correlated features and suggests dropping redundant ones.
    Follows the exact same workflow as missing values/outliers.
    """
    numeric_cols = df.select_dtypes(include=['float64', 'int64']).columns
    if len(numeric_cols) < 2:
    ...
    # We store tuples of (column_to_drop, correlated_with_column, correlation_value)
    high_corr_data = []
    high_corr_cols = set()
    
    for col in upper.columns:
        # Check if this column has any correlation > threshold with previous columns
    ...
    # Display details
    # We group by the column we might want to drop
    ...
    # === GLOBAL SUGGESTION ===
    # Strategy: Suggest 'drop' for all columns identified in the upper triangle loop (redundant features)
    ...
    # Apply user's plan
    # Sort drop IDs in reverse order to avoid index shifting issues (though we drop by name)
    ...
    for col_id in ids_to_drop:
        ...
        updated_df.drop(columns=[col_name], inplace=True)
        # Update info immediately to prevent errors if logic gets complex
        ...
        while True:
            ...
            if choice == '1':
                ...
                for col_id in extra_drops:
                    col_name = df_info[df_info['id'] == col_id]['column_name'].iloc[0]
                    # Check if it still exists (it should)
...
def handle_categorical_encoding(df, df_info):
    """
    1. Identifies object/categorical columns.
    2. Suggests: 
       - 'one_hot' if unique values < 10 (Low cardinality)
       - 'label' if unique values >= 10 (High cardinality)
    ...
    if approve == 'y':
        ...
        user_input = input("Your plan: ").strip()
        # You can reuse parse_outlier_plan logic or write a simple parser here
        # reusing your existing parsing structure:
        user_plan = {'one_hot': set(), 'label': set(), 'skip': set()}
        parts = [p.strip() for p in user_input.split(';') if p.strip()]
        for part in parts:
            ...
            # Extract IDs (simplified version of your existing parsers)
            try:
                id_str = part.split('-')[1]
                ids = set()
    ...
    # Process One-Hot (get_dummies)
    # We collect all one-hot cols to process at once usually, but here we do iterative to match ID system
    # Note: get_dummies changes the shape/columns completely, so we do it last or carefully.
    
    # Let's do Label Encoding first as it doesn't change column count
    ...
    # Update df_info because columns changed
    new_info = get_df_info(updated_df)
    return updated_df, new_info

def handle_feature_scaling(df, df_info):
    """
    1. Identifies numeric columns.
    2. Suggests:
       - 'standard' (StandardScaler) if data looks normally distributed (skew between -1 and 1)
       - 'minmax' (MinMaxScaler) if data is skewed or needs fixed range [0,1]
    3. User approves or modifies.
    """
    # Exclude Target Variable usually, but for EDA tool we ask user or scale all features
    ...
    for _, row in scale_info.iterrows():
        col_id = row['id']
        # If skew is high, MinMax is often safer to bound it, or if user prefers Standard, that's fine too.
        # Heuristic: Standard for Normal-ish, MinMax for Skewed/Bounded
        if abs(row['skew']) < 1:
...
# Update your main block to ask after outliers
    ...
    if df is not None:
        ...
        if do_bivariate == 'y':
            ...
            if do_drop_corr == 'y':
                ...
                df, info = handle_high_correlation(df, info, threshold)
        
        # After Correlation Handling
        
        # 1. Feature Encoding
        do_encode = input("\nDo you want to encode categorical columns (One-Hot/Label)? (y/n): ").strip().lower()
        if do_encode == 'y':
            df, info = handle_categorical_encoding(df, info)
            # Info must be refreshed here because One-Hot encoding adds new columns
        ...
        if do_scale == 'y':
            df, info = handle_feature_scaling(df, info)
